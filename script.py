...
class Project(object):
	...
	def AP_metric(self,data,days_considered):
		data_dict={}
		session = self.session
		header = self.header
		for key,values in data.items():
			...
			for item in values:
				if item is not None:
					...
					bool_value = self.get_uptime(key,days_considered)       #check to see if uptime is more than 7 days
	...
	def get_uptime(self,site_id,days_considered):
		session = self.session
		header = self.header
		url = '{}/sites/{}/stats/devices'.format(API_URL,site_id)
		response = session.get(url, headers=header)
...
def main():
	...
	data={}
	days_considered = int(input("Enter number of days for data calculation: "))
	print("\n")
	...
	data_list=(site_object.AP_metric(data,days_considered))
